...
class ContextIDSetValue(Node):
    ...
    def setup(self):
        ...
        self.add_input("context_id_item", socket_type="context_id_item", optional=True)
    ...
    async def run(self, state: GraphState):
        ...
        path: str | None = self.normalized_input_value("path")
        value = self.require_input("value")
        scene: "Scene" = active_scene.get()

        # either path or context_id_item must be set
        if not context_id_item and not path:
            raise InputValueError(
                self, "path", "Either path or context_id_item must be set"
            )

        if not context_id_item:
            context_id_item = await context_id_item_from_string(path, scene)
