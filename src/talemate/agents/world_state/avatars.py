...
class AvatarMixin:
    ...
    def add_actions(cls, actions: dict[str, AgentAction]):
        actions["avatars"] = AgentAction(
            ...
            label="Character Avatars",
            icon="mdi-image-auto-adjust",
            description="Avatar-related features for world state management.",
            config={
                "selection_frequency": AgentActionConfig(
                    type="number",
                    label="Selection Frequency",
                    title="Avatar Selection",
                    description="Setting a frequency of 1 or higher will cause the world state agent to determine the most appropriate avatar for the character based on the current moment in the scene. A frequency of 1 means this is evaluated with every new message. 0 = Never, 10 = Every 10th message.",
                    value=1,
                    min=0,
                    max=10,
                    step=1,
