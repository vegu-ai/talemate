                ...
                <v-form class="overflow-y-auto">
                    <v-card class="overflow-y-auto">
                        ...
                        <v-card-text>
                            ...
                            <v-checkbox v-if="isSentenceTransformer" v-model="config.embeddings[selected[0]].trust_remote_code" label="Trust Remote Code" @update:model-value="setPresetChanged(selected[0])"></v-checkbox>

                            <v-select v-if="isLocal" v-model="config.embeddings[selected[0]].device" :items="devices" label="Device" @update:model-value="setPresetChanged(selected[0])"></v-select>

                            <v-slider thumb-label="always" density="compact" v-model="config.embeddings[selected[0]].distance" min="0.1" max="10.0" step="0.1" label="Distance" @update:model-value="setPresetChanged(selected[0])"></v-slider>

                            <v-slider thumb-label="always" density="compact" v-model="config.embeddings[selected[0]].distance_mod" min="1" max="1000" step="10" label="Distance Mod" @update:model-value="setPresetChanged(selected[0])"></v-slider>

                            <v-select v-model="config.embeddings[selected[0]].distance_function" :items="distanceFunctions" label="Distance Function" @update:model-value="setPresetChanged(selected[0])"></v-select>


                            <v-row>
                                <v-col cols="3">
                                    <v-checkbox v-model="config.embeddings[selected[0]].fast" label="Fast" @update:model-value="setPresetChanged(selected[0])"></v-checkbox>
                                </v-col>
                                <v-col cols="6">
                                    <v-checkbox v-if="isLocal" v-model="config.embeddings[selected[0]].gpu_recommendation" label="GPU Recommendation" @update:model-value="setPresetChanged(selected[0])"></v-checkbox>
                                </v-col>
                                <v-col cols="3">
                                    <v-checkbox v-model="config.embeddings[selected[0]].local" label="Local" @update:model-value="setPresetChanged(selected[0])"></v-checkbox>
                                </v-col>
                            </v-row>
