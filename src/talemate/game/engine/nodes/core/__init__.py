...
def get_ancestors_with_forks(graph: nx.DiGraph, node_id: str) -> set[str]:
    ...
    for ancestor_id in ancestors:
        ...
        forked_nodes.update(desc for desc in descendants 
                           if desc not in ancestors and desc != node_id)
...
class GraphState(pydantic.BaseModel):
    ...
    def flattened(self) -> dict:
        try:
            ...
            log.error("error dumping stack (stack probably contains circular references)", error=e)
            self.stack = []
            return {"stack": []}
    ...
    def get_node_property(self, node:"NodeBase", name:str) -> Any:
        return self.data.get(self.node_property_key(node, name), node.properties.get(name, UNRESOLVED))
...
class NodeBase(pydantic.BaseModel):
    ...
    def model_dump(self, **kwargs):
        ...
        if 'properties' in data:
            data['properties'] = {
                k: None if v is UNRESOLVED else v 
    ...
    def get_property_field(self, name: str) -> PropertyField:
        ...
        if not hasattr(self.__class__, "Fields"):
            type_str = PYTHON_TYPE_TO_STRING.get(str(type(self.properties.get(name, "")))) or "str"
    ...
    def get_input_values(self) -> dict[str, Any]:
        ...
        names = set([socket.name for socket in self.inputs] + list(self.properties.keys()))
    ...
    def check_is_available(self, state:GraphState) -> bool:
        ...
        for socket in ungrouped_sockets:
            if socket.source is None or socket.source.deactivated or socket.value is UNRESOLVED:
                if self.get_property(socket.name) is UNRESOLVED:
                    
                    if state.verbosity >= NodeVerbosity.VERBOSE:
                        log.warning(f"Node {self.title} input {socket.name} is not available, missing socket {socket.name}")
        ...
        for group_sockets in grouped_sockets.values():
            ...
            for socket in group_sockets:
                ...
                if (socket.source and not socket.source.deactivated and socket.value is not UNRESOLVED) or self.get_property(socket.name) is not UNRESOLVED:
                    group_has_active = True
                    break
            
            if not group_has_active:
                if state.verbosity >= NodeVerbosity.VERBOSE:
                    log.warning(f"Node {self.title} group {group_sockets[0].group} is not available")
        ...
        is_available = any(has_active_path(socket, visited.copy()) 
                        for socket in self.outputs)
    ...
    async def run(self, state: GraphState):
        ...
        for idx, socket in enumerate(self.outputs):
            if idx != route_to:
                ...
                print(f"Setting output {socket.name} to {self.get_input_value('input')}")
                self.set_output_values({
                    socket.name: self.get_input_value("input")
                })
...
RegistryNode = Annotated[
    NodeBase,
    pydantic.WrapValidator(validate_node)
]
...
class Graph(NodeBase):
        ...
        try:
            if save_state.get():
                ...
                data["edges"] = {
                    ...
                    and all(input_id.split(".")[0] not in dropped_node_ids for input_id in input_ids)
                }
    ...
    def ensure_connections(self):
        ...
        for output_socket_id, input_socket_ids in self.edges.items():
            ...
            for input_socket_id in input_socket_ids:
                ...
                if not input_socket:
                    raise ValueError(f"Input socket {input_socket_name} not found in node {input_node_id}")
            ...
            for socket in node.inputs:
                for output_socket_id, input_socket_ids in self.edges.items():
                    ...
                    if socket.id in input_socket_ids:
                        output_socket = self.sockets[f"{output_node_id}.{output_socket_name}"]
    ...
    def connect(self, output_socket: Socket | str, input_socket: Socket | str):
        ...
        if not output_socket or not input_socket:
            log.warning("Could not connect sockets", output_socket=output_socket, input_socket=input_socket)
            return
    ...
    def build(self) -> nx.DiGraph:
        ...
        for output_socket_id, input_socket_ids in self.edges.items():
            ...
            output_node_title = self.nodes[output_node_id].title
            
            for input_socket_id in input_socket_ids:
                input_node_id, _ = input_socket_id.split(".",1)
                
                input_node_title = self.nodes[input_node_id].title
    ...
    async def node_state_pop(
        ...
        node_exec = NodeState(node, state, start_time=prev_state.start_time, end_time=prev_state.end_time)
    ...
    async def get_node(self, fn_filter:Callable=None, require_unique:bool=True) -> NodeBase:
        """
    ...
    async def get_nodes_connected_to(self, node: NodeBase, fn_filter:Callable=None) -> list[NodeBase]:
        """
        ...
        return [self.nodes[node_id] for node_id in predecessors if fn_filter(self.nodes[node_id])]
    ...
    async def execute_to_node(
        ...
        with GraphContext(outer_state, self) as state:
            ...
            await self._execute_inner(subgraph, state, emit_state=emit_state, run_isolated=run_isolated)
    ...
    async def _execute_inner(
        ...
        try:
            ...
            for node in self.input_nodes:
                ...
                socket_value = state.outer.get_node_socket_value(socket.source.node, socket.source.name)
            ...
            for chain in chains:
                ...
                for node_id in sorted_nodes:
                    ...
                    if isinstance(node, Output):
                        socket = self.get_output_socket(node.get_property("output_name"))
                        if socket:
                            socket.value = node.get_input_socket("value").value
                            state.outer.set_node_socket_value(self, socket.name, socket.value)
                            
                            if verbosity == NodeVerbosity.VERBOSE:
                                log.debug(f"Setting output value for {socket.full_id} to {socket.value}")
...
class Loop(Graph):
    ...
    exit_condition: Callable = pydantic.Field(default_factory=lambda: lambda state: False, exclude=True)
    ...
    async def execute(self, outer_state: GraphState, state_values:dict=None, run_isolated:bool=False):
        """Execute the graph in topological order"""
        graph = self.build()
                ...
                while True:
                    ...
                    try:
                        ...
                        try:
                            ...
                            log.error("Error in on_loop_start", exc=exc, traceback=traceback.format_exc())
                        except LoopBreak:
                            BREAK_LOOP = True
                        except LoopContinue:
                            pass
                    ...
                    for chain in chains:
                        ...
                        for node_id in sorted_nodes:
                            try:
                                ...
                                BREAK_LOOP = True
                                break
                            except (ExitScene, ResetScene, RestartSceneLoop, StopModule) as exc:
                                raise
                            except Exception as exc:
                                try:
                                    await self.handle_error(state, exc)
                                    await self.on_loop_error(state, exc)
                                    log.error("Error in Loop", graph=self.title, graph_cls=self.__class__, exc=exc, traceback=traceback.format_exc())
                                    ...
                                    log.error("Error in on_loop_error", exc=traceback.format_exc())
                                BREAK_LOOP = True
                                break
...
class Listen(Graph):
    ...
    async def execute_from_event(self, event:object):
        ...
        try:
            ...
            await self.node_state_pop(node_state, self, state, error=traceback.format_exc())
            raise exc
