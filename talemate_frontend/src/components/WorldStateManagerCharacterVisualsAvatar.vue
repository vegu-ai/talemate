    ...
    <div>
        ...
        >
            ...
            <template #badges="{ asset }">
                <VisualAssetBadge v-if="defaultAvatarId === asset.id" icon="mdi-check" label="Default" />
                <VisualAssetBadge v-if="currentAvatarId === asset.id" icon="mdi-account" label="Current" color="primary" position="top-right" />
            </template>
            <template #card-overlay="{ asset }">
                <VisualAssetBadge v-if="!hasTags(asset)" icon="mdi-tag-off" label="No Tags" color="delete" position="bottom-overhang" />
            </template>
            <template #menu="{ asset }">
                <v-list-item
        ...
        <VisualAssetGenerateCards
            ...
            new-label="portrait"
            create-warning-subject="new portraits"
        ...
        <VisualAssetGenerateDialog
            ...
            batch-info-text="Each prompt will create a separate generation using the same reference image and settings. Generations will be queued in the Visual Library. Tags can be added using {tag} syntax in each prompt."
...
export default {
    ...
    computed: {
        ...
        referenceConfig() {
            return {
                visType: VIS_TYPE.CHARACTER_PORTRAIT,
                preferredId: this.defaultAvatarId,
                fallbackId: this.character?.cover_image,
            };
        },
        variationPromptHint() {
            return this.shouldUseVariationForInitial
                ? 'e.g., generate close up of the character head with a neutral expression. Add tags using {tag} syntax, e.g., {happy} {portrait}'
                : 'e.g., change the expression to sad. Add tags using {tag} syntax, e.g., {sad} {portrait}';
        },
        initialVariationPrompt() {
            return 'generate close up of the character\'s head with a neutral expression';
        },
        initialNewPrompt() {
            return 'Create a portrait with a neutral expression';
        },
    },
    watch: {
        character: {
    ...
    methods: {
        cardStateClass(asset) {
            return {
                'current': this.defaultAvatarId === asset.id,
                'active': this.currentAvatarId === asset.id,
            };
        },
