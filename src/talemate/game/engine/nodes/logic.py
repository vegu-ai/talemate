...
class LogicalRouter(Node):
    ...
    async def run(self, state: GraphState):
        ...
        if state.verbosity == NodeVerbosity.VERBOSE:
            log.debug(f"LogicalRouter {self.title} result: {result}", flags=active_flags, result=result, input_values=self.get_input_values())
            
            for socket in self.inputs:
                if socket.source:
                    log.debug(f"LogicalRouter {self.title} Input {socket.name} source: {socket.source.node.title}.{socket.source.name} value: {socket.value} ! {socket.source.value}")
                else:
                    log.debug(f"LogicalRouter {self.title} Input {socket.name} source: NOT CONNECTED")
