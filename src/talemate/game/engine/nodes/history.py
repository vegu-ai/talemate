    ...
    async def run(self, state: GraphState):
        ...
        if emit_message:
            if isinstance(message, scene_message.CharacterMessage):
                emit("character", message, character=scene.get_character(message.character_name))
                ...
                emit("director", message, character=scene.get_character(message.character_name) if message.character_name else None)
    ...
    async def run(self, state: GraphState):
        ...
        for mt in message_type:
            if mt not in MESSAGES:
                raise InputValueError(self, "message_type", f"Message type {mt} is not valid")
        ...
        message = scene.last_message_of_type(message_type, max_iterations=max_iterations,stop_on_time_passage=stop_on_time_passage, **filters)
...
class ContextHistory(Node):
    ...
    async def run(self, state: GraphState):
        ...
        min_dialogue_length = self.require_number_input("min_dialogue_length", types=(int,))
